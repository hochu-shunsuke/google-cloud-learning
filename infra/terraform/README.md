...
サービス群で利用する GCP リソースを Terraform でコード化します。  
...
既に作成済みのリソースを Terraform 管理下に移行します。  
...
```bash
# 各サービスのイメージをビルド（Cloud Run がイメージを参照するため）
gcloud run deploy image-upload-api --source ../../services/upload-api --region asia-northeast1 --no-traffic
gcloud run deploy bq-subscriber --source ../../services/bq-subscriber --region asia-northeast1 --no-traffic
